public struct ExpenseReport {
    ...
    func findValues(from startIndex: Int, sum: Int, lineItemCount: Int) -> [Int]? {
        ...
        // Recursively iterate the remaining items with one fewer items and a sum decreased by the value.
        // Once this returns non-nil, we have found the entries that sum to the desired total.
        for (index, value) in lineItems[startIndex...].enumerated() {
            guard let values = findValues(from: index + 1, sum: sum - value, lineItemCount: lineItemCount - 1) else {
                continue
            }
            return values + [value]
        }
