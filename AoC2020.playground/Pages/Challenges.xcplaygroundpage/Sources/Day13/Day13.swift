...
public struct ShuttleHandler {
   ...
   public init(_ input: [String]) {
        guard let estimatedTimestampString = input.first,
              let estimatedTimestamp = Int(estimatedTimestampString)
        else
        {
            fatalError()
        }
    ...
    public var earliestBusProduct: Int {
        ...
        guard let busID = closestBusID else { fatalError() }
        
        return (busID.busID * busID.offset)
    }

    public var earliestSynchronizedDeparture: Int {
        guard let busIDs = input.last?.components(separatedBy: ",").map({ Int($0) }) else { fatalError() }
